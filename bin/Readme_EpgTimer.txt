...
　　・設定ボタン
　　　　EpgTimer_Bon、EpgTimerSrvの各種設定を行うダイアログを表示します。
...
　　・再生ボタン
　　　　保存ファイルに関連付けされているアプリケーションがある場合、起動し
　　　　ます。
...
　●番組表
　　読み込んでいるEPGデータをラテ欄で表示します。
　　ドラッグすることでスクロールが可能です。
　　番組をダブルクリックすることで、予約登録、または予約変更ダイアログ
　　が開きます。
　　TVTest連携の設定を行っていると、サービス名をダブルクリックすること
　　でTVTestのチャンネルを切り替えることができます。
　　詳細は後述の■TVTest連携■を参照。
...
　EPGデータの検索を行う場合の条件です。
　検索、自動予約登録などで使用されます。
　※映像、音声絞り込みは検索の場合のみ有効です。自動予約登録の条件に
　することはできません。
...
　・時間絞り込み
　　検索対象の番組開始時間を指定できます。コンボボックスから条件を
　　選択し、リストに追加します。
　　◆「追加」を押してリストに追加しないと有効になりません。
...
　・映像絞り込み
　　映像の形式を指定できます。コンボボックスから条件を選択し、リス
　　トに追加します。
　　◆「追加」を押してリストに追加しないと有効になりません。

　・音声絞り込み
　　音声の形式を指定できます。コンボボックスから条件を選択し、リス
　　トに追加します。
　　◆「追加」を押してリストに追加しないと有効になりません。
...
　　　したい場合に使用します。
　　　出力先と出力PlugInを指定して、リストに追加してください。
　　　複数指定することで全フォルダに出力します。
　　　◆「追加」を押してリストに追加しないと有効になりません。
　　　※出力PlugInの設定は全予約共通となります。変更するとすべての予約
　　　　に影響が出ます。
...
　　　　・視聴時はViewを起動する。
　　　　　録画モードが視聴の場合、Viewに指定されているアプリも起動します。
...
　　　・録画設定のデフォルト値を設定
　　　　録画設定のデフォルト値を設定できます。
　　　　検索、予約追加、iEPGからの予約追加時のデフォルト設定値となります。
...
　　・フォント
　　　番組表示で表示する文字のフォントをサイズを指定することができます。

　●番組表設定２タブ
　　・表示対象サービス
　　　表示するサービスと、表示順を指定することができます。
...
　TVTestの起動とチャンネル切り換えが行えるようになります。
　予約録画時にチューナーが使用されていた場合、TVTestを自動的に終了する
　こともできます。
　◆TVTest側でPlugInにチェックを入れて有効にしてください。

　TVTest側では、EpgDataCap_Bonが使用するBonDriverと同じものが使用でき
　るように設定されている必要があります。
...
　登録した条件に合致する番組を自動的に予約登録します。追加と検索から条
　件を登録できます。自動登録のタイミングはEPGデータの読み込みを行った時
　になります。
...
　録画終了時に、他に予約録画が行なわれていた場合、全ての予約録画が終わっ
　てから実行されます。
...
＝＞EpgTimerSrv.exeをサービス登録し、EpgTimer.exeが起動していないと表
　　示されません。
...
複数録画フォルダ：2つ上指定時の2つめ以降を\t区切りの「パス*PlugIn名」で
連続録画：0:しない 1:する
部分受信同時出力：0:しない 1:する
強制使用チューナーID：0:自動
予約追加状態：0
...
・予約一覧に背景赤や黄色の項目がある
＝＞時間が重なって録画できない予約があります。赤が実行されない予約。
　　黄が赤の予約と時間が重なる予約になります。
　　灰色は録画モードが無効にされたものです。
