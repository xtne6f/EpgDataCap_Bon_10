...
void CTwitterManager::CreateTextChgResReload(wstring& text, RESERVE_DATA* oldInfo, RESERVE_DATA* newInfo)
{
	...
	WCHAR buff[1024] = L"";
	GetPrivateProfileString(L"TWITTER_MACRO", L"MACRO1", L"●EPG読み込み：$ServiceNameOLD$ $SDYYYYOLD$/$SDMOLD$/$SDDOLD$ $STHOLD$:$STMOLD$ ～ $ETHOLD$:$ETMOLD$ EventID:0x$EID16OLD$ → $SDYYYYNEW$/$SDMNEW$/$SDDNEW$ $STHNEW$:$STMNEW$ ～ $ETHNEW$:$ETMNEW$ EventID:0x$EID16NEW$ $TitleNEW$", buff, 1024, iniAppPath.c_str());
...
void CTwitterManager::CreateTextChgResRec(wstring& text, RESERVE_DATA* oldInfo, RESERVE_DATA* newInfo, EPGDB_EVENT_INFO* eventInfo)
{
	...
	WCHAR buff[1024] = L"";
	GetPrivateProfileString(L"TWITTER_MACRO", L"MACRO2", L"●追従：$ServiceNameOLD$ $SDYYYYOLD$/$SDMOLD$/$SDDOLD$ $STHOLD$:$STMOLD$ ～ $ETHOLD$:$ETMOLD$ → $SEYMDHMNEW$ $TitleNEW$", buff, 1024, iniAppPath.c_str());
	...
	if( eventInfo->StartTimeFlag == 0 ){
		macroData.newSYMDHM = L"未定";
		macroData.newSYMDHM28 = L"未定";
		macroData.newEYMDHM = L"未定";
		macroData.newEYMDHM28 = L"未定";
		macroData.newSEYMDHM = L"未定";
		macroData.newSEYMDHM28 = L"未定";
	}else{
		...
		if( eventInfo->DurationFlag == 0 ){
			...
			Format(macroData.newSEYMDHM, L"%d/%d/%d %d:%d ～ 未定",
				eventInfo->start_time.wYear,
				eventInfo->start_time.wMonth,
				eventInfo->start_time.wDay,
				eventInfo->start_time.wHour,
			...
			Format(macroData.newSEYMDHM28, L"%d/%d/%d %d:%d ～ 未定",
				t28TimeSPG.wYear,
				t28TimeSPG.wMonth,
				t28TimeSPG.wDay,
				t28TimeSPG.wHour,
...
void CTwitterManager::CreateTextAddRes(wstring& text, RESERVE_DATA* resInfo)
{
	...
	WCHAR buff[1024] = L"";
	GetPrivateProfileString(L"TWITTER_MACRO", L"MACRO3", L"●予約追加：$ServiceNameNEW$ $SDYYYYNEW$/$SDMNEW$/$SDDNEW$ $STHNEW$:$STMNEW$ ～ $ETHNEW$:$ETMNEW$ $TitleNEW$", buff, 1024, iniAppPath.c_str());
...
void CTwitterManager::CreateTextRecStart(wstring& text, RESERVE_DATA* resInfo)
{
	...
	WCHAR buff[1024] = L"";
	GetPrivateProfileString(L"TWITTER_MACRO", L"MACRO4", L"●録画開始：$ServiceNameNEW$ $SDYYYYNEW$/$SDMNEW$/$SDDNEW$ $STHNEW$:$STMNEW$ ～ $ETHNEW$:$ETMNEW$ $TitleNEW$", buff, 1024, iniAppPath.c_str());
...
void CTwitterManager::CreateTextRecStop(wstring& text, REC_FILE_INFO* resInfo)
{
	...
	WCHAR buff[1024] = L"";
	GetPrivateProfileString(L"TWITTER_MACRO", L"MACRO5", L"●録画終了：$ServiceNameNEW$ $SDYYYYNEW$/$SDMNEW$/$SDDNEW$ $STHNEW$:$STMNEW$ ～ $ETHNEW$:$ETMNEW$ $Result$ Drops:$Drops$ Scrambles:$Scrambles$ $TitleNEW$", buff, 1024, iniAppPath.c_str());
